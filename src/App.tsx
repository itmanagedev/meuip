...
export default function App() {
  ...
  const [lgResults, setLgResults] = useState<any[]>([]);
  ...
  const handleLookingGlass = async () => {
    ...
    const locations = [
      { name: 'EUA, Ashburn', country_code: 'US', rtt_base: 120 },
      { name: 'Brasil, São Paulo', country_code: 'BR', rtt_base: 15 },
      { name: 'Reino Unido, Londres', country_code: 'GB', rtt_base: 180 },
      { name: 'Japão, Tóquio', country_code: 'JP', rtt_base: 280 },
      { name: 'Alemanha, Frankfurt', country_code: 'DE', rtt_base: 195 },
      { name: 'Austrália, Sydney', country_code: 'AU', rtt_base: 320 }
    ];

    try {
      // Simulate multi-location ping
      for (const loc of locations) {
        // Just mock some RTT jitter based on base
        const rtt = (loc.rtt_base + (Math.random() * 20)).toFixed(2);
        setLgResults(prev => [...prev, { ...loc, rtt, status: 'Active' }]);
        ...
        <div className="mt-6">
          <AnimatePresence mode="wait">
            ...
            {activeTab === 'looking-glass' && (
              ...
              >
                <div className="bg-card-bg border border-border-dim rounded-2xl p-8 text-center space-y-8">
                   <div className="w-20 h-20 bg-brand-accent/10 rounded-3xl flex items-center justify-center mx-auto border border-brand-accent/20">
                      <Server className="w-10 h-10 text-brand-accent" />
                   </div>
                   <div className="space-y-2">
                      <h2 className="text-3xl font-black text-white uppercase italic tracking-tighter">Looking Glass Global</h2>
                      <p className="text-text-dim text-sm max-w-md mx-auto">Verifique a acessibilidade e latência do seu IP a partir de nós estratégicos em todo o mundo.</p>
                   </div>
                   ...
                   <div className="flex max-w-md mx-auto gap-2">
                      <input 
                         ...
                         placeholder="Insira o IP para teste global"
                         className="flex-grow bg-bg-dark border border-border-dim rounded-xl px-5 py-3 text-sm font-mono focus:border-brand-accent outline-none"
                      />
                      <button 
                         onClick={handleLookingGlass}
                         disabled={isLgRunning || !lgTarget}
                         className="bg-brand-accent px-6 rounded-xl font-black text-[10px] uppercase tracking-widest hover:brightness-110 active:scale-95 disabled:opacity-50"
                      >
                         {isLgRunning ? <Activity className="w-4 h-4 animate-spin" /> : 'Ping Global'}
                      </button>
                   </div>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                   {lgResults.map((res, i) => (
                      <motion.div 
                         ...
                         key={res.name} 
                         className="bg-card-bg border border-border-dim p-5 rounded-2xl flex items-center justify-between group hover:border-brand-accent/40"
                      >
                         <div className="flex items-center gap-4">
                            <div className="w-10 h-10 bg-bg-dark rounded-xl flex items-center justify-center border border-border-dim font-bold text-xs text-brand-accent group-hover:bg-brand-accent group-hover:text-white">
                               {res.country_code}
                            </div>
                            <div>
                               <div className="text-[12px] font-bold text-white">{res.name}</div>
                               <div className="text-[9px] text-text-dim uppercase tracking-widest">Router @ iTmanage Edge</div>
                            </div>
                         </div>
                         <div className="text-right">
                            <div className="text-lg font-black font-mono text-brand-success">{res.rtt}ms</div>
                            <div className="text-[9px] text-brand-success font-bold uppercase">Connected</div>
                         </div>
                      </motion.div>
                   ))}
                </div>
